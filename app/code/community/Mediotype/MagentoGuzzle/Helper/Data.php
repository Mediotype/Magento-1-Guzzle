<?php
/**
 * Migrated to Magento's helper system
 * @author  Joel Hart
 */
class Mediotype_MagentoGuzzle_Helper_Data extends Mage_Core_Helper_Abstract{

    const GUZZLE_FUNCTIONS_VERSION =  Mediotype_MagentoGuzzle_Model_ClientInterface::VERSION;
    ...
    function request($method, $url, array $options = array())
    {
        static $client;
        if (!$client) {
            $client = new Mediotype_MagentoGuzzle_Model_Client();
        }
     ...
     *
     * @param Mediotype_MagentoGuzzle_Model_ClientInterface $client   Client used to send the requests
     ...
     */
    function batch(Mediotype_MagentoGuzzle_Model_ClientInterface $client, $requests, array $options = array())
    {
        ...
        foreach ($hash as $request) {
            if ($hash[$request] instanceof Mediotype_MagentoGuzzle_Model_Event_CompleteEvent) {
                $hash[$request] = $hash[$request]->getResponse();
            } elseif ($hash[$request] instanceof Mediotype_MagentoGuzzle_Model_Event_ErrorEvent) {
    ...
    function uri_template($template, array $variables)
    {
        ...
        static $uriTemplate;
        if (!$uriTemplate) {
            $uriTemplate = new Mediotype_MagentoGuzzle_Model_UriTemplate();
        }
