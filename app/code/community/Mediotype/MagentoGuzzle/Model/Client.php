...
class Mediotype_MagentoGuzzle_Model_Client implements Mediotype_MagentoGuzzle_Model_ClientInterface
{
    ...
    /** @var Mediotype_MagentoGuzzle_Model_Message_MessageFactoryInterface Request factory used by the client */
    private $messageFactory;

    /** @var Mediotype_MagentoGuzzle_Model_Adapter_AdapterInterface */
    private $adapter;

    /** @var Mediotype_MagentoGuzzle_Model_Adapter_ParallelAdapterInterface */
    private $parallelAdapter;

    /** @var Mediotype_MagentoGuzzle_Model_Url Base URL of the client */
    private $baseUrl;
    ...
    public function __call($name, $arguments)
    {
        ...
        return $this->helper->deprecation_proxy(
            $this,
            $name,
            $arguments,
    ...
    public function getDefaultOption($keyOrPath = null)
    {
        return $keyOrPath === null
            ? $this->defaults
            : $this->helper->get_path($this->defaults, $keyOrPath);
    }

    public function setDefaultOption($keyOrPath, $value)
    {
        $this->helper->set_path($this->defaults, $keyOrPath, $value);
    }
    ...
    public function send(Mediotype_MagentoGuzzle_Model_Message_RequestInterface $request)
    {
        $transaction = new Mediotype_MagentoGuzzle_Model_Adapter_Transaction($this, $request);
        try {
    ...
    {
        if (!($requests instanceof Mediotype_MagentoGuzzle_Model_Adapter_TransactionIterator)) {
            $requests = new Mediotype_MagentoGuzzle_Model_Adapter_TransactionIterator($requests, $this, $options);
        }
    ...
    private function buildUrl($url)
    {
        if (!is_array($url)) {
            ...
            return $this->helper->uri_template($url[0], $url[1]);
        }

        return (string) $this->baseUrl->combine(
            $this->helper->uri_template($url[0], $url[1])
        );
    }
     ...
     *
     * @return Mediotype_MagentoGuzzle_Model_Adapter_ParallelAdapterInterface
     */
    private function getDefaultParallelAdapter()
    {
        return extension_loaded('curl')
            ? new Mediotype_MagentoGuzzle_Model_Adapter_Curl_MultiAdapter($this->messageFactory)
            : new Mediotype_MagentoGuzzle_Model_Adapter_FakeParallelAdapter($this->adapter);
    }
    ...
    {
        if (extension_loaded('curl')) {
            $this->parallelAdapter = new Mediotype_MagentoGuzzle_Model_Adapter_Curl_MultiAdapter($this->messageFactory);
            $this->adapter = function_exists('curl_reset')
                ? new Mediotype_MagentoGuzzle_Model_Adapter_Curl_CurlAdapter($this->messageFactory)
                : $this->parallelAdapter;
            if (ini_get('allow_url_fopen')) {
                $this->adapter = new Mediotype_MagentoGuzzle_Model_Adapter_StreamingProxyAdapter(
                    $this->adapter,
                    new Mediotype_MagentoGuzzle_Model_Adapter_StreamAdapter($this->messageFactory)
                );
            }
        } elseif (ini_get('allow_url_fopen')) {
            $this->adapter = new Mediotype_MagentoGuzzle_Model_Adapter_StreamAdapter($this->messageFactory);
        } else {
    ...
    private function configureBaseUrl(&$config)
    {
        if (!isset($config['base_url'])) {
            ...
            $this->baseUrl = Mediotype_MagentoGuzzle_Model_Url::fromString($config['base_url']);
        }
    }
    ...
    private function configureAdapter(&$config)
    {
        if (isset($config['message_factory'])) {
            ...
            $this->messageFactory = new Mediotype_MagentoGuzzle_Model_Message_MessageFactory();
        }
