 ...
 */
class Mediotype_MagentoGuzzle_Model_QueryParser
{
    private $duplicates;
     ...
     *
     * @param Mediotype_MagentoGuzzle_Model_Query       $query       Query object to populate
     * @param string      $str         Query string to parse
     * @param bool|string $urlEncoding How the query string is encoded
     */
    public function parseInto(Mediotype_MagentoGuzzle_Model_Query $query, $str, $urlEncoding = true)
    {
        if ($str === '') {
            return;
        }
        ...
        if (!$this->numericIndices) {
            $query->setAggregator(Mediotype_MagentoGuzzle_Model_Query::phpAggregator(false));
        } elseif ($this->duplicates) {
            $query->setAggregator(Mediotype_MagentoGuzzle_Model_Query::duplicateAggregator());
        }
    }
    ...
    private static function getDecoder($type)
    {
        ...
        } elseif ($type == Mediotype_MagentoGuzzle_Model_Query::RFC3986) {
            return 'rawurldecode';
        } elseif ($type == Mediotype_MagentoGuzzle_Model_Query::RFC1738) {
            return 'urldecode';
        } else {
