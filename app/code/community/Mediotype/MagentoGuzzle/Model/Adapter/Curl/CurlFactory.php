 ...
 */
class Mediotype_MagentoGuzzle_Model_Adapter_Curl_CurlFactory
{
    /**
     * Creates a cURL handle based on a transaction.
     *
     * @param Mediotype_MagentoGuzzle_Model_Adapter_TransactionInterface $transaction Holds a request and response
     * @param Mediotype_MagentoGuzzle_Model_Message_MessageFactoryInterface $messageFactory Used to create responses
     * @param null|resource $handle Optionally provide a curl handle to modify
     *
     * @return resource Returns a prepared cURL handle
     * @throws Mediotype_MagentoGuzzle_Model_Exception_AdapterException when an option cannot be applied
     */
    public function __invoke(
        ...
        $mediator = new Mediotype_MagentoGuzzle_Model_Adapter_Curl_RequestMediator($transaction, $messageFactory);
    ...
    protected function getDefaultOptions(
        ...
        Mediotype_MagentoGuzzle_Model_Adapter_Curl_RequestMediator $mediator
    ) {
        $url = $request->getUrl();
    ...
    private function applyMethod(Mediotype_MagentoGuzzle_Model_Message_RequestInterface $request, array &$options)
    {
    ...
    private function applyBody(Mediotype_MagentoGuzzle_Model_Message_RequestInterface $request, array &$options)
    {
    ...
    private function applyHeaders(Mediotype_MagentoGuzzle_Model_Message_RequestInterface $request, array &$options)
    {
    ...
    private function add_ssl_key(
        ...
        if (!file_exists($value)) {
            throw new Mediotype_MagentoGuzzle_Model_Exception_AdapterException("SSL private key not found: {$value}");
        }
    ...
    private function add_stream(
        ...
        throw new Mediotype_MagentoGuzzle_Model_Exception_AdapterException('cURL adapters do not support the "stream"'
    ...
    private function add_save_to(
        ...
        $mediator->setResponseBody(is_string($value)
            ? new Mediotype_MagentoGuzzle_Model_Streams_LazyOpenStream($value, 'w')
            : Mediotype_MagentoGuzzle_Model_Streams_Stream::factory($value));
    }
