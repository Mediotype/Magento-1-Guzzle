 ...
 * Post file upload
 */
class Mediotype_MagentoGuzzle_Model_Post_PostFile implements Mediotype_MagentoGuzzle_Model_Post_PostFileInterface
{
    private $name;
    private $filename;
    private $content;
    ...
    private function prepareContent($content)
    {
        ...
        if (!($this->content instanceof Mediotype_MagentoGuzzle_Model_Streams_StreamInterface)) {
            $this->content = Mediotype_MagentoGuzzle_Model_Streams_Stream::factory($this->content);
        } elseif ($this->content instanceof Mediotype_MagentoGuzzle_Model_Post_MultipartBody) {
    ...
    private function prepareFilename($filename)
    {
        ...
        if (!$this->filename &&
            $this->content instanceof Mediotype_MagentoGuzzle_Model_Streams_MetadataStreamInterface
        ) {
    ...
    private function prepareDefaultHeaders()
    {
        ...
        if (!$this->hasHeader('Content-Type')) {
            $this->headers['Content-Type'] = Mediotype_MagentoGuzzle_Model_Mimetypes::getInstance()
