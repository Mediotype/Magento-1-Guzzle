 ...
 */
class Mediotype_MagentoGuzzle_Model_Url
{
    private $scheme;
    private $host;
    private $port;
    ...
    /** @var Mediotype_MagentoGuzzle_Model_Query Query part of the URL */
    private $query;
     ...
     *
     * @return Mediotype_MagentoGuzzle_Model_Url
    ...
    public static function fromString($url)
    {
        ...
        if ($parts['query'] || 0 !== strlen($parts['query'])) {
            $parts['query'] = Mediotype_MagentoGuzzle_Model_Query::fromString($parts['query']);
        }
     ...
     * @param Mediotype_MagentoGuzzle_Model_Query|array|string $query    Query string of the URL
    ...
    public function __construct(
        ...
        $port = null,
        $path = null,
        Mediotype_MagentoGuzzle_Model_Query $query = null,
        $fragment = null
    ) {
        $this->scheme = $scheme;
        $this->host = $host;
        ...
        if (!$query) {
            $this->query = new Mediotype_MagentoGuzzle_Model_Query();
        } else {
            $this->setQuery($query);
        }
        $this->setPath($path);
     ...
     *
     * @param Mediotype_MagentoGuzzle_Model_Query|string|array $query Query string value to set. Can
    ...
    {
        if ($query instanceof Mediotype_MagentoGuzzle_Model_Query) {
            $this->query = $query;
        } elseif (is_string($query)) {
            $this->query = Mediotype_MagentoGuzzle_Model_Query::fromString($query);
        } elseif (is_array($query)) {
            $this->query = new Mediotype_MagentoGuzzle_Model_Query($query);
        } else {
